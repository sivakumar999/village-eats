...
import { ArrowLeft, Minus, Plus, Trash2, MapPin, ShoppingBag, Edit2, Check } from 'lucide-react';
...
export default function Cart() {
  const { 
    ...
    getSubtotal, getDeliveryFee, getTotal, currentRestaurantId 
  } = useCart();
    ...
    return (
      <div className="min-h-screen bg-background">
        <header className="sticky top-0 z-50 bg-card border-b border-border">
          <div className="container mx-auto px-4 py-4">
            <div className="flex items-center gap-4">
              <Link to="/home" className="p-2 rounded-full hover:bg-muted transition-colors">
        ...
        <div className="flex flex-col items-center justify-center py-20 px-4">
          ...
          <p className="text-muted-foreground text-center mb-6">Add some delicious items from our restaurants</p>
          <Link to="/home">
    ...
    <div className="min-h-screen bg-background pb-32">
      <header className="sticky top-0 z-50 bg-card border-b border-border">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <Button 
              variant="ghost" size="sm" onClick={clearCart}
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        <section className="space-y-4 mb-8">
          {items.map((item) => (
            <div key={item.foodItem.id} className="bg-card rounded-xl p-4 shadow-card animate-fade-in">
              <div className="flex gap-4">
                <img src={item.foodItem.image} alt={item.foodItem.name} className="w-20 h-20 rounded-lg object-cover" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-semibold text-foreground truncate">{item.foodItem.name}</h3>
                      </div>
                      <p className="text-sm text-muted-foreground mt-1">₹{item.foodItem.price} each</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-1 bg-muted rounded-lg">
                      <Button variant="quantity" size="sm" onClick={() => updateQuantity(item.foodItem.id, item.quantity - 1)} className="h-8 w-8">
                    ...
                    <p className="font-bold text-foreground">₹{item.foodItem.price * item.quantity}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="bg-card rounded-xl p-4 shadow-card mb-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              {isChangingLocation ? (
                <Select value={currentLocation?.id} onValueChange={(value) => {
                  const location = availableLocations.find(l => l.id === value);
                  if (location) { setCurrentLocation(location); setIsChangingLocation(false); }
                }}>
                ...
                {isEditingAddress ? (
                  <div className="flex gap-2">
                    <Input value={deliveryAddress} onChange={(e) => setDeliveryAddress(e.target.value)}
                      placeholder="Enter your full delivery address..." className="flex-1" />
                    <Button size="icon" variant="ghost" onClick={() => setIsEditingAddress(false)} className="shrink-0">
                      <Check className="h-4 w-4" />
                    </Button>
                  </div>
                ) : (
                  <div onClick={() => setIsEditingAddress(true)}
